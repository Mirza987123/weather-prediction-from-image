...
validation_label = np.load("../concat100/validation_label.npy")  # Load validation labels from .npy file  # Load validation labels from .npy file  # Load validation labels from .npy file  # Load validation labels from .npy file  # Load validation labels from .npy file
...
vd = T.separate_data(validation_data, validation_label)  # Separate validation data and labels into classes  # Separate validation data and labels into classes  # Separate validation data and labels into classes  # Separate validation data and labels into classes  # Separate validation data and labels into classes

# number of class
num_classes = 5  # Cloudy,Sunny,Rainy,Snowy,Foggy

# for example if label is 4 converts it [0,0,0,0,1]
validation_label = np_utils.to_categorical(validation_label, num_classes)  # Convert labels to categorical format  # Convert labels to categorical format  # Convert labels to categorical format  # Convert labels to categorical format  # Convert labels to categorical format


# loads trained model and architecture
model = load_model("modelsCNN/size100/trainedModelE20.h5")  # Load the trained CNN model  # Load the trained CNN model  # Load the trained CNN model  # Load the trained CNN model


# -------predicting part-------
y = model.predict_classes(validation_data, verbose=0)  # Predict classes for validation data  # Predict classes for validation data  # Predict classes for validation data  # Predict classes for validation data
...
for i in range(len(vd)):  # Iterate through each class for accuracy calculation  # Iterate through each class for accuracy calculation
    ...
    acc = T.get_accuracy_of_class(v_label, y)  # Calculate accuracy for the current class  # Calculate accuracy for the current class
